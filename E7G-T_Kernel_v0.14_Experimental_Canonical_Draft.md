...
version: "0.14-experimental-draft"
release_label: "v0.14-experimental-REC1-draft"
date: "2026-09-15"
revision: "REC1"
...
status: "Experimental canonical draft; explicit formal calculus with bounded executable reference models"
base_version: "0.13-experimental-RWP1-draft"
...
normativity: "Part I defines v0.14 draft scope and conditional EEC-Q, SF, CFS, RGP, WPC and REC profile obligations. REC/0.1 applies only when explicitly selected and constrains a bounded information-flow computation; it does not establish source truth, domain-rule validity or improved AI performance. WPC-Evolution and WPC-Distributed remain separate optional capabilities. Part II preserves the UC5 reference text and its existing normative/informative distinctions. Appendices V and VI retain limited predecessor implementation and validation accounts unchanged. Draft canonical means a candidate authoritative successor under review; it does not amend a predecessor, assert stable adoption, production readiness or maturity-gate completion."
ai_use: "Treat E7G-T as a geometry-first modelling language with experimental constructive, reciprocal and checkable-reasoning profiles. Apply EEC-Q, SF, CFS, RGP, WPC or REC only when explicitly selected. Under REC/0.1 distinguish claim content, information status, source truth and action authority; retain support and refutation independently; reject hidden scope, temporal and modality expansion; require explicit rules and a replayable witness; and abstain when the selected reliance policy is not satisfied. Do not reinterpret EEC-Q coefficients as evidence weights. A passing REC trace proves only conformance of the declared bounded information flow, not factual truth, domain validity, intelligence improvement, physical ontology or production readiness."
---

# E7G-T Kernel v0.14 — Experimental Draft
...
> **v0.14 working definition.** E7G-T is a geometry-first modelling language and practical calculus for constructing, extending, combining, transforming and viewing bounded relational entities, and for examining their preservation, loss, histories and inquiry-relative phases. Its experimental profiles distinguish constitution from representation and may additionally evaluate finite claim–evidence systems through declared rules. A REC result retains support and refutation independently, exposes conflict and staleness, rejects hidden scope or modality expansion, and carries a replayable witness. Such a result establishes bounded information-flow conformance rather than external truth.
...
| Part | Content | Role |
|---|---|---|
| I, §§X.0–X.20 | v0.14 definitions, rules, examples, implementation contract and migration | Current experimental draft |
...
References written **UC5 §n** point into Part II. References written **§X.n** point into Part I. References written **§W.n** point into WPC/0.2 in §X.19; references written **§C.n** point into REC/0.1 in §X.20. The source's historical front matter is recorded before Part II; only the front matter at the beginning of this document describes the v0.14 draft.

# Part I — Executable Extension, Recursive Structure and Reciprocal Constitution
...
**MUST**, **MUST NOT**, **SHOULD** and **MAY** have the obligation meanings used in UC5. An EEC-Q requirement applies only to an implementation or derivation declaring that profile. SF, CFS, RGP, WPC and REC obligations apply only when their respective profiles are selected. WPC/0.1 and WPC/0.2 remain distinct pinned profiles; WPC evolution and distributed obligations require their additional capabilities. REC/0.1 constrains only its declared finite claim–evidence computation. Each supported model and capability scope must be declared. Ordinary E7G-T analyses do not acquire a requirement to use rational coefficients, finite graphs, cancellation, recursive frames, whole–part constitution or REC.

Part I governs the v0.14 draft status and the new profiles. Part II governs inherited concepts except for the explicit refinements listed in §X.14. Nothing here promotes UC5's informative observation, temporal-orientation, topology or relative-support pilots to the normative constitutional core.
...
| Term | Meaning in v0.13 | What does not follow automatically |
|---|---|---|
...
| UC5 element | v0.14 draft treatment | Compatibility consequence |
|---|---|---|
...
3. For a new v0.14 draft artefact, declare `kernel=0.14-experimental-draft`, every selected profile and the supported model edition.
...
Promotion of this draft should require a published grammar and interchange schema, an implementation covering each declared capability, independent reproduction of positive and negative cases, at least one useful end-to-end program, and a reviewed compatibility account. Performance or quantum claims require separate evidence appropriate to those claims.

The v0.14 draft retains the v0.13 constructive and reciprocal profiles and proposes REC/0.1 as a separately selectable reasoning-audit capability. The bounded REC-B1 model checks declared finite information flow; it does not supply a natural-language semantic parser, general theorem prover, source-truth oracle, domain authority engine or empirical proof of improved AI reasoning. A full language, OS, hardware mechanism, general WPC runtime and general-purpose verifier remain future engineering or research work. This draft is an implementable foundation and review target rather than a claim that the destination has already been reached.
...
Added in revision **REC1**, 2026-09-15. The following standalone profile is reproduced here as part of the v0.14 draft. The versioned package supplies the bounded REC-B1/0.1 evaluator, schema, fixtures, tests, validation record and conformance matrix.
...
The following reference body is reproduced unchanged from the repository's **E7G-T_Kernel_v0.11_UC5_Unified_Public_Reference_Specification.md** predecessor. Its internal section numbering, older version strings, public-reference anchors and informative module labels are historical source content. Read its concepts together with the v0.14 draft scope in §X.0 and the explicit migration refinements in §X.14. This inclusion makes the document self-contained; it does not claim that the predecessor's maturity gates have been passed.

Source metadata, retained for provenance rather than as v0.13 front matter:

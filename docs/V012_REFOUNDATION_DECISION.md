...
Retain v0.12-experimental/CFS1, its three opt-in profiles, bounded models and predecessor material. No kernel rewrite is justified. Consumer adapters must not be promoted into the kernel until two products demonstrate compatible reuse.
...
Publish only consumer-driven conformance contracts and independent reproductions; do not add theory to solve product integration problems.

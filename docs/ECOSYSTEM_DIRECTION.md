...
The experimental canonical source for new work is v0.15/MSC1. It incorporates v0.13 reciprocal whole–part work, v0.14 optional REC/0.1 and the later optional MSC/0.1 formal proposal, while keeping their conformance and maturity limits explicit. v0.12.1, v0.13 and v0.14 remain immutable source editions; existing consumers retain their pins pending separate migration. REC checks bounded declared information flow and produces a replayable witness; MSC checks model-relative multi-scope coherence. Neither establishes external truth, physical hierarchy or improved AI performance. See [the successor decision](CANONICAL_SUCCESSOR_DECISION.md).

The central `E7-Ecosystem/DIRECTION.md` currently pins v0.12.1 for its
product architecture. Its adoption decision remains separate from this source
edition and requires its own reviewed change before ecosystem migration.

REC consumers must keep claim content, evidence polarity, information status, source truth and action authority distinct. They must pin source editions, temporal and semantic scope, modalities, dependence groups, rules and decision policy. MSC consumers pin scope carriers, maps, criteria, partial domains, limits and editions. EEC-Q coefficients must not be reused as evidence weights. E7C remains incomplete and does not automatically inherit the v0.15 source edition.

...
subtitle: "Executable Extension, Composition and Recursive Generative–Projective Structures"
version: "0.12.1-experimental"
release_label: "v0.12.1-experimental-RGP2"
date: "2026-09-13"
revision: "RGP2"
...
base_version: "0.12-experimental"
...
optional_recursive_profile: "RGP/0.1"
symbolic_reference_model: "IC/0.1"
example_model: "FG3/0.1"
language_convention: "British English"
normativity: "Part I defines v0.12.1 scope and conditional EEC-Q, SF, CFS and RGP profile obligations. Part II preserves the UC5 reference text and its existing normative/informative distinctions. Appendices V and VI contain limited v0.12 implementation and validation accounts retained unchanged. Canonical means the current authoritative v0.12.1 experimental reference; it does not assert stable adoption, production readiness or maturity-gate completion."
ai_use: "Treat E7G-T as a geometry-first modelling language with an experimental constructive and executable direction. Distinguish entities, descriptions, configurations, formal combinations, quoted constructions, views and phases. Apply EEC-Q, SF, the optional CFS combined family-state profile, or the optional RGP recursive generative-projective profile only when explicitly selected. Distinguish SF set-valued families and realisation from EEC-Q signed combinations. Under RGP distinguish hosting, generation, encoding, decoding, projection, inheritance, composition, rank, regime and access scope. A source-retaining encoding is not a view; a lineage reference is not a self-contained encoding. Preserve joint alternatives, explicit interfaces, domain conditions and loss accounts. Do not replace the constructive objective with evidence reporting alone. Do not infer physical dimensions, external ontology, quantum implementation, computational advantage or general correctness from the formal calculus or example model."
---

# E7G-T Kernel v0.12.1 — Experimental
...
| Part | Content | Role |
|---|---|---|
| I, §§X.0–X.18 | v0.12.1 definitions, rules, examples, implementation contract and migration | Current experimental specification |
...
References written **UC5 §n** point into Part II. References written **§X.n** point into Part I. The source's historical front matter is recorded before Part II; only the front matter at the beginning of this document describes v0.12.1.
...
| RGP/0.1 | Optional recursive generative-projective structures with typed hosting, generation, whole-bearing encoding, decoding, projection, inheritance and composition | Added in §X.18; structural specification without a claim of external instantiation |
...
**MUST**, **MUST NOT**, **SHOULD** and **MAY** have the obligation meanings used in UC5. An EEC-Q requirement applies only to an implementation or derivation declaring that profile. SF requirements apply only when SF is selected; CFS requirements apply only when the combined profile is selected; RGP requirements apply only when the recursive generative-projective profile is selected. Each supported model and capability scope must be declared. Ordinary E7G-T analyses do not acquire a requirement to use rational coefficients, finite graphs, cancellation, recursive frames or this execution model.

Part I governs v0.12.1 version status and the new profiles. Part II governs inherited concepts except for the explicit refinements listed in §X.14. Nothing here promotes UC5's informative observation, temporal-orientation, topology or relative-support pilots to the normative constitutional core.
...
| Term | Meaning in v0.12.1 | What does not follow automatically |
|---|---|---|
...
| UC5 element | v0.12.1 treatment | Compatibility consequence |
|---|---|---|
...
The working definition at the beginning of v0.12.1 makes construction, computation and recursive generative-projective structure explicit. It extends the project's direction while retaining the UC5 distinction between source, representation, phase and evidence. It does not turn every E7G-T use into an algebraic execution or every projection into an encoding.
...
3. For a new v0.12.1 artefact, declare `kernel=0.12.1-experimental`, every selected profile and the supported model edition.
...
The v0.12.1 deliverables include the experimental calculus, its worked constructions, the retained runnable v0.12 finite example models, the optional SF symbolic-family profile, the optional CFS family-state composition, and the optional RGP recursive generative-projective structural profile with SR4 exact whole-bearing encoding. A full language, OS, hardware mechanism and general-purpose verifier remain future engineering or research work. This is an implementable foundation for that work rather than a claim that the destination has already been reached.
...
## X.18 Optional recursive generative–projective structures — RGP/0.1

Added in revision **RGP1**, 2026-09-13, and extended with exact whole-bearing encoding in revision **RGP2** on the same date. RGP/0.1 is an optional structural profile for modelling finite hierarchies in which a bounded construction may host other constructions, generate locally differentiated successors, carry source-retaining encodings, expose restricted projections, transmit declared invariants and participate in a composite that can itself recur at a higher rank.
...
None of these relations implies another. A hosted object need not have been generated by its host. A generated object need not be hosted by its generator. An encoding need not be a projection or view. A projection need not be a component of its source. Decoding a reference through external retrieval is not self-contained reconstruction. Inheritance does not imply exact identity. Composition does not imply loss of component identity.
...
\[
L=\langle\Sigma,B,G,I,A,E,D,\Pi,K,M\rangle,
\]
...
The following obligations govern RGP/0.1:
...
Execution or derivation MUST stop with `unsupported`, `undetermined`, `domain_error` or a UC5 stop record when a required generator, interface, invariant witness, projection loss account, decoder, transition rule or access policy is unavailable.
...
This example exercises recursive generation, shared dependence, invariant inheritance, exact whole-bearing encoding, access-limited projection, composition and lifting without assigning domain-specific meaning to the hierarchy.
...
The following reference body is reproduced unchanged from the repository's current **E7G-T_Kernel_v0.11_UC5_Unified_Public_Reference_Specification.md** predecessor. Its internal section numbering, older version strings, public-reference anchors and informative module labels are historical source content. Read its concepts together with the v0.12.1 scope in §X.0 and the explicit migration refinements in §X.14. This inclusion makes the document self-contained; it does not claim that the predecessor's maturity gates have been passed.

Source metadata, retained for provenance rather than as v0.12.1 front matter:
...
# v0.12.1 provenance and closing anchor
...
Version 0.12.1 is the patch-level successor to v0.12-experimental. It retains the SF1 symbolic-family work and the CFS1 combined family-state profile, CG3 prototype and validation companion. Revision RGP2 retains the recursive generative-projective structural profile introduced in RGP1 and adds first-class encoding, decoding and exact SR4 whole-reconstruction semantics in §X.18 without changing the recorded v0.12 executable validation counts or companion-source hashes. The 2026-09-07 v0.12 draft remains preceding development material. The UC5 body between the preservation markers is unchanged from the repository predecessor. No changes have been made to that predecessor or the ecosystem file. All new worked configurations are constructed examples; this specification includes no private collaborator implementation or experimental data.

**Working anchor:** construct entities with declared boundaries; retain the dependence among their alternatives; distinguish hosting, generation, encoding, decoding, projection, inheritance and composition; extend through typed interfaces; distinguish a source-retaining portion from its local view; promote whole constructions explicitly; and test every claimed abstraction law on its actual domain.

End of **E7G-T Kernel v0.12.1-experimental — Executable Extension, Composition and Recursive Generative–Projective Structures**.

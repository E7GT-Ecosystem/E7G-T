"""Versioned IR for the opt-in B2 success-only two-map sequence."""
...
from e7c_success_sequence_b2 import EDITION as SOURCE_EDITION, FIRST, admit, evaluate_sequence
from e7c_success_sequence_checker_b2 import check_sequence

EDITION = "E7-IR/0.3-success-sequence-B2-provisional"
CAPABILITY = "core.sequence.success/0.3"
MAX_BYTES = 1_000_000
...
def _instruction(source, static):
    ...
    first = env["maps"]["total_identity"]
    next_map = env["maps"][continuation]
    node = {"operator": CAPABILITY, "first_map": "total_identity",
...
def execute(package):
    ...
    first_atom = {"dimension": "evidence", "payload": canonical_key({
        ...
        "map_declaration": "total_identity", "map_edition": first["map_edition"],
        "outcome_extension": first["outcome_extension"]})}
    if not append(first_atom, {"declaration": "total_identity", "event": "map_attempt"}):
        return finish(limit())
    first_table = tables["total_identity"]
    if first_table["capability"] is not True:
        return finish(outcome("unsupported", "map:total_identity"))
    if first_table["obligation"] != "resolved":
        return finish(outcome("undetermined", "map-domain:total_identity"))
...
def compare_replay(package):
    ...
    if actual != expected:
        raise SequenceIRAdmissionError("source B2 and IR observation mismatch")
    return {"status": "matched_selected_success_sequence", "ir_result": actual,

"""Opt-in E7C-B2 success sequencing of one total map and a named map.
...
EDITION = "E7C-B2/0.1-success-sequence-provisional"
WITNESS_EDITION = "e7c-b2-success-sequence-witness-0.1-provisional"
FIRST = {"tag": "apply", "declaration": "total_identity",
         "arg": {"tag": "var", "name": "source_config"}}
CONTINUATIONS = {"strict_normalise": "strict", "total_identity": "total"}
...
def admit(document):
    try:
        ...
        if (first["domain_policy"] != "total" or
                next_map["domain_policy"] != CONTINUATIONS[term["then_map"]] or
                first_type.tag != "outcome" or
                first_type.args[0] != Checker(env).check({"tag": "var", "name": "source_config"}).type or
        ...
        outer = Checker(env).check({"tag": "apply", "declaration": term["then_map"],
                                     "arg": {"tag": "var", "name": "source_config"}})

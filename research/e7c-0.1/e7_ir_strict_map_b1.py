"""Provisional two-instruction IR for one admitted strict map over var.
...
EDITION = "E7-IR/0.1-strict-map-var-B1-provisional"
CAPABILITY = "core.apply.strict/0.1"
MAX_BYTES = 1_000_000
...
def lower(document):
    ...
    term = source.term
    if (term.get("tag") != "apply" or term.get("declaration") != "strict_normalise" or
            term.get("arg") != {"tag": "var", "name": "source_config"} or
            source.environment["maps"]["strict_normalise"]["domain_policy"] != "strict"):
        raise MapIRAdmissionError("only strict_normalise(var source_config) is admitted")
...
def parse(encoded):
    ...
    try:
        ...
        if canonical_bytes(package) != encoded or set(package) != {
            ...
            "argument_ir", "source_document_digest", "id"
        ...
        term = source.term
        if (term != {"tag": "apply", "declaration": "strict_normalise",
                     "arg": {"tag": "var", "name": "source_config"}} or
                source.environment["maps"]["strict_normalise"]["domain_policy"] != "strict"):

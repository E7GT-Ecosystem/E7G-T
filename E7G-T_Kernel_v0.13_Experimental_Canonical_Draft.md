...
subtitle: "Executable Extension, Recursive Structures and Reciprocal Whole–Part Constitution"
version: "0.13-experimental-draft"
release_label: "v0.13-experimental-RWP1-draft"
date: "2026-09-14"
revision: "RWP1"
...
base_version: "0.12.2-experimental"
...
normativity: "Part I defines v0.13 draft scope and conditional EEC-Q, SF, CFS, RGP and WPC profile obligations. WPC/0.2 core obligations apply only when that profile is selected; WPC-Evolution and WPC-Distributed are separate optional capabilities. Part II preserves the UC5 reference text and its existing normative/informative distinctions. Appendices V and VI retain limited v0.12 implementation and validation accounts unchanged. Draft canonical means a candidate authoritative successor under review; it does not amend v0.12.2, assert stable adoption, production readiness or maturity-gate completion."
ai_use: "Treat E7G-T as a geometry-first modelling language with an experimental constructive and executable direction. Distinguish entities, descriptions, configurations, formal combinations, quoted constructions, views and phases. Apply EEC-Q, SF, CFS, RGP or WPC only when explicitly selected. Under WPC/0.2 distinguish constitutive presentation, semantic whole and materialised assembly; distinguish constitution from encoding and representation from current operation; evaluate coverage, global admission and exact reciprocal closure separately. Treat all-portions SR4 as an optional retention capability, not a prerequisite for constitution. State-changing local expression produces a proposal or event rather than a hidden mutation. Apply evolution, distributed consistency, authority and confidentiality obligations only under their declared capability boundaries. Preserve alternatives, histories, scope, unknowns and failure outcomes. Do not infer physical dimensions, external ontology, quantum implementation, universal completeness, computational advantage or general correctness from a structural law or bounded example."
---

# E7G-T Kernel v0.13 — Experimental Draft

**Executable Extension, Recursive Structures and Reciprocal Whole–Part Constitution**

**Authors:** Alexander Gregory Wingate and Oleksandr Razinkov
**Copyright:** © 2026 Alexander Gregory Wingate and Oleksandr Razinkov
**Licence:** [CC BY-SA 4.0](https://creativecommons.org/licenses/by-sa/4.0/)

> **v0.13 working definition.** E7G-T is a geometry-first modelling language and practical calculus for constructing, extending, combining, transforming and viewing bounded relational entities, and for examining their preservation, loss, histories and inquiry-relative phases. Its experimental profiles additionally distinguish a whole constituted by participating local states, relations, dependencies and history from representations that allow that whole to be reconstructed. Admitted local and relational changes may jointly constitute a successor whole whose representations are then renewed, weakened explicitly or marked stale.
...
| Part | Content | Role |
|---|---|---|
| I, §§X.0–X.19 | v0.13 definitions, rules, examples, implementation contract and migration | Current experimental draft |
...
References written **UC5 §n** point into Part II. References written **§X.n** point into Part I. References written **§W.n** point into the reciprocal whole–part profile in §X.19. The source's historical front matter is recorded before Part II; only the front matter at the beginning of this document describes the v0.13 draft.
...
**MUST**, **MUST NOT**, **SHOULD** and **MAY** have the obligation meanings used in UC5. An EEC-Q requirement applies only to an implementation or derivation declaring that profile. SF requirements apply only when SF is selected; CFS requirements apply only when the combined profile is selected; RGP requirements apply only when the recursive generative-projective profile is selected. WPC/0.1 and WPC/0.2 are distinct pinned profiles. WPC/0.2 core obligations apply only when that profile is selected; its evolution and distributed obligations apply only when their additional capabilities are selected. Each supported model and capability scope must be declared. Ordinary E7G-T analyses do not acquire a requirement to use rational coefficients, finite graphs, cancellation, recursive frames, whole–part constitution or this execution model.

Part I governs the v0.13 draft status and the new profiles. Part II governs inherited concepts except for the explicit refinements listed in §X.14. Nothing here promotes UC5's informative observation, temporal-orientation, topology or relative-support pilots to the normative constitutional core.
...
| UC5 element | v0.13 draft treatment | Compatibility consequence |
|---|---|---|
...
The working definition at the beginning of v0.13 makes construction, computation, recursive generative-projective structure and reciprocal whole–part constitution explicit. It retains the UC5 distinction between source, representation, phase and evidence. It does not turn every E7G-T use into an algebraic execution, every constituent into an SR4 encoder or every composition into a uniquely admitted whole.
...
3. For a new v0.13 draft artefact, declare `kernel=0.13-experimental-draft`, every selected profile and the supported model edition.
...
The v0.13 draft retains the v0.12 executable calculus and profiles, RGP/0.2 and legacy WPC/0.1, and proposes WPC/0.2 reciprocal whole–part constitution with optional evolution and distributed capabilities. A full language, OS, hardware mechanism, general WPC runtime and general-purpose verifier remain future engineering or research work. This draft is an implementable foundation and review target rather than a claim that the destination has already been reached.
...
The following reference body is reproduced unchanged from the repository's current **E7G-T_Kernel_v0.11_UC5_Unified_Public_Reference_Specification.md** predecessor. Its internal section numbering, older version strings, public-reference anchors and informative module labels are historical source content. Read its concepts together with the v0.13 draft scope in §X.0 and the explicit migration refinements in §X.14. This inclusion makes the document self-contained; it does not claim that the predecessor's maturity gates have been passed.
...
# v0.13 draft provenance and closing anchor

| Source | Role | SHA-256 of release bytes |
|---|---|---|
| E7G-T Kernel v0.12.2-experimental-WPC1 | Immediate predecessor and legacy WPC/0.1 base | `c36225ef601b82eb025872f53e7a881bbc257d4668df3ae4733113c32bf21515` |
...
Version 0.13 is drafted as the successor to v0.12.2-experimental. It retains the EEC-Q, SF, CFS, RGP/0.2 and WPC/0.1 definitions and the existing FG3, IC and CG3 validation records. Revision RWP1 proposes WPC/0.2 with separate constitutive presentation, semantic whole and materialised assembly; an explicit extraction-to-constitution reconstruction path; normalised reciprocal closure; optional all-portions SR4; local expression discipline; typed membership and participation; and independent capability reporting. It also proposes separately selectable WPC-Evolution/0.1 and WPC-Distributed/0.1 modules. The reviewed bounded allocation example passed 34 internal checks, but no complete WPC/0.2 runtime or independent reproduction is claimed. The UC5 body between the preservation markers is unchanged from the repository predecessor. No predecessor, ecosystem file or product pin is modified by this draft.

**Working anchor:** construct entities with declared boundaries; retain dependence, local state, relations and history; constitute the semantic whole before evaluating its representations; distinguish occurrence, expression, payload and authority; reconstruct a materialised assembly through extraction, unique constitution and payload agreement; renew encodings only after an admitted successor is committed; and test every completeness, closure, evolution and availability claim on its declared domain.

End of **E7G-T Kernel v0.13-experimental-RWP1-draft — Executable Extension, Recursive Structures and Reciprocal Whole–Part Constitution**.
